...
def checkRedirect_vk( msg ):
	...
	if not config.getCell( "vk_" + chatid ) is None:
		...
		userName = getUserName( msg['last_message'] )
		mbody = msg['last_message'].get( 'text' ).lower()

		# Чтобы при событии не посылалось пустое сообщение
		if checkEvents( msg, chatid ) is None:
		    if re.findall(r"((рас)?печат((а(ть|л(а|о|и)|й|ю|е(шь|т|м|е)|я)|ы(в(а(ть|л(а|о|и)|й|ю|е(шь|т|м|е)|я)|шь|т|м|е)|й(те)?|ая?|ое?))|ь))|копи[яиюе]|принте[ра]|скан|отсканить|отсканировать|микронаушник|микронаушника|микронаушники", mbody):
    ...
    while True:
        try:
            ...
            rawMessages = module.vk.messages.getConversations(
                filter="unread", count=config.getCell("vk_msgForPick")
            )["items"]
            if not rawMessages:
                continue

            for chat in rawMessages:
                msg = chat["conversation"]["peer"]
                if checkRedirect_vk(chat) or config.getCell(
                    "vk_markAsReadEverything"
                ):
